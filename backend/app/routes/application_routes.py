...
def apply(body: ApplyRequest):
    ...
    resume = resumes.find_one({"id": body.resumeId}, {"_id": 0, "skills": 1})
    if not resume:
        raise HTTPException(status_code=404, detail="Resume not found")

    job = jobs.find_one({"id": body.jobId}, {"_id": 0, "required_skills": 1})
    if not job:
    ...
    match_score = round(len(matching) / len(required) * 100) if required else 0
